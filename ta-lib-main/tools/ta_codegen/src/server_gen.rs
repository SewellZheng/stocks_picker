...
/// Generate `ta_func_private.h` — forward declarations for `TA_*_Private` functions.
/// These are double-only, with custom signatures (extra params like EMA's k factor).
...
pub fn generate_c_private_header(funcs: &[FuncDef]) -> String {
    ...
    s.push_str(" * Private function declarations (double-only, custom signatures).\n");
    s.push_str(" */\n\n");
    s.push_str("#ifndef TA_FUNC_PRIVATE_H\n");
    s.push_str("#define TA_FUNC_PRIVATE_H\n\n");

    for func in funcs {
        if !func.has_explicit_private || func.private_extra_params.is_empty() {
            continue;
        }
        let upper = func.name.to_uppercase();
